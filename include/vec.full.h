#ifndef VEC_VEC_FULL_H
#define VEC_VEC_FULL_H
#include "vec.h"
...
constexpr void vec<T,Allocator>::resize(size_t new_cap){
    ...
    if(new_cap < cap){
        ...
        std::copy(arr, arr + std::min(sz, new_cap), copy); ///TODO replace copy with swaps
        allocator.deallocate(arr, cap);
        cap = new_cap;
        arr = copy;
        return;
