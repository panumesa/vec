...
vec<T, Allocator>::vec(size_t size, const T &val, Allocator alloc):sz(size), cap(size),
                                                                   allocator(alloc),
                                                                   arr(std::allocator_traits<Allocator>::allocate(
                                                                           allocator, size)) {
    for (int i = 0; i < sz; ++i) {
...
vec<T, Allocator>::vec(InputIterator begin, InputIterator end, Allocator alloc):
        sz(std::distance(begin, end)), cap(0), allocator(alloc),
        arr(std::allocator_traits<Allocator>::allocate(allocator, sz)) {
    for (; begin != end; ++begin) {
...
constexpr typename vec<T, Allocator>::iterator
vec<T, Allocator>::insert(typename vec<T, Allocator>::iterator pos, InputIt first, InputIt last) {
    ...
    if (sz + len_of_gap <= cap) {
        ...
        for (int i = start; i < sz; ++i) {
            std::allocator_traits<Allocator>::construct(allocator, newarr + i + len_of_gap,
...
//        arr[i + len_of_shift] = arr[i];
        try {
            std::allocator_traits<Allocator>::construct(allocator, arr + i + len_of_shift,
                                                        std::move_if_noexcept(arr[i]));
        }
        catch (...) {
            ~vec<T, Allocator>();
...
constexpr typename vec<T, Allocator>::iterator vec<T, Allocator>::erase(const_iterator first, const_iterator last) {
    ...
    for (std::ptrdiff_t i = last_index; i < sz; ++i) {
        std::allocator_traits<Allocator>::construct(allocator, arr + i - shift,
                                                    std::move_if_noexcept(arr[i]));//TODO try catch
    }
    sz -= shift;
    if (4 * sz < cap) shrink((cap + 1) / 2);
